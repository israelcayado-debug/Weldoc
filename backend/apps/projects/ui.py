...
def project_list(request):
    ...
    items = models.Project.objects.select_related("client").all().order_by("name")
    if q:
    ...
    clients = models.Client.objects.all().order_by("name")
    return render(
        request,
        "projects/list.html",
...
def project_detail(request, pk):
    ...
    documents = document_models.Document.objects.filter(project=item).count()
